...
namespace fullsail_ai { namespace algorithms {
	...
	class PathSearch
	{
		...
		struct Comp
		{
			bool operator()(PlannerNode* one, PlannerNode* two)
			{
				return (one->totalcost > two->totalcost);
			}
		};
		...
		TileMap *map;
		SearchNode* end;
		std::map<Tile*, SearchNode*> search;
		std::map<SearchNode*, PlannerNode*> planmap;
		PriorityQueue<PlannerNode*, Comp> queue;
		bool trueend;
		std::vector<Tile const*> temp;

		float estimate(SearchNode* start, SearchNode* finish);
		void makeSolution();
